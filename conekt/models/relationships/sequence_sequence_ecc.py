...
class SequenceSequenceECCAssociation(db.Model):
    ...
    gene_family_method_id = db.Column(
        db.Integer, db.ForeignKey("gene_family_methods.id", ondelete="CASCADE")
    )
    query_network_method_id = db.Column(
        db.Integer, db.ForeignKey("expression_network_methods.id", ondelete="CASCADE")
    )
    target_network_method_id = db.Column(
        db.Integer, db.ForeignKey("expression_network_methods.id", ondelete="CASCADE")
    )

    gene_family_method = db.relationship(
        "GeneFamilyMethod",
        lazy="joined",
        backref=db.backref(
            "ecc_as_family_method", lazy="dynamic", passive_deletes=True
        ),
    )
    ...
    query_expression_network_method = db.relationship(
        ...
        lazy="joined",
        backref=db.backref("ecc_as_query_method", lazy="dynamic", passive_deletes=True),
    )
    target_expression_network_method = db.relationship(
        ...
        lazy="joined",
        backref=db.backref(
            "ecc_as_target_method", lazy="dynamic", passive_deletes=True
        ),
    )
        ...
        for n, sequences in networks.items():
            new_data = SequenceSequenceECCAssociation.query.filter(
                and_(
                    ...
                    SequenceSequenceECCAssociation.gene_family_method_id == family,
                    SequenceSequenceECCAssociation.query_id
                    != SequenceSequenceECCAssociation.target_id,
                )
            ).all()
            ...
            for nd in new_data:
                ...
                if not any(
                    d["source"] == nd.target_sequence.name
    ...
    def get_ecc_pair_network(ecc_id):
        ...
        query_network = (
            association.query_sequence.network_nodes.filter_by(
                method_id=association.query_network_method_id
            )
            .first_or_404()
            .network
        )
        target_network = (
            association.target_sequence.network_nodes.filter_by(
                method_id=association.target_network_method_id
            )
            .first_or_404()
            .network
        )
        ...
        for n in target_network_data:
            ...
            edges.append(
                {
                    "source": association.target_sequence.name,
                    "target": gene_name,
                    "link_score": n["link_score"] if "link_score" in n else 0,
                    "edge_type": "expression",
                    "ecc_pair_color": "#3D3",
    ...
    def get_ecc_multi_network(gf_method_id, sequence_ids):
        ...
        """
        associations = (
            SequenceSequenceECCAssociation.query.filter(
                SequenceSequenceECCAssociation.gene_family_method_id == gf_method_id
            )
            .filter(
                and_(
                    SequenceSequenceECCAssociation.query_id.in_(sequence_ids),
        ...
        for a in associations:
            query_network = (
                a.query_sequence.network_nodes.filter_by(
                    method_id=a.query_network_method_id
                )
                .first_or_404()
                .network
            )
            target_network = (
                a.target_sequence.network_nodes.filter_by(
                    method_id=a.target_network_method_id
                )
                .first_or_404()
                .network
            )

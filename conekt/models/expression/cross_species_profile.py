...
class CrossSpeciesExpressionProfile:
    def __init__(self):
        ...
        """
        self.condition_tissue = ConditionTissue.query.filter(
            ConditionTissue.in_tree == 1
        ).all()

        # Way to merge various (potentially incomplete) lists and preserve the order (as good as possible)
        merged_conditions = list(
            merge(*[json.loads(ct.data)["order"] for ct in self.condition_tissue])
        )

        # Make list unique keeping the element with the highest index (reason for double reverse)
        self.conditions = list(
            reversed(  # reverse again and convert to list
                list(
                    OrderedDict.fromkeys(  # make list unique
                        reversed(merged_conditions)
                    )  # reverse input
                )
            )
        )
    ...
    def get_data(self, *sequence_ids):
        ...
        """
        profiles = (
            ExpressionProfile.query.filter(
                ExpressionProfile.sequence_id.in_(list(sequence_ids))
            )
        ...
        for p in profiles:
            if p.species_id in self.species_to_condition.keys():
                current_profile = p.tissue_profile(
                    self.species_to_condition[p.species_id].id
                )
                ...
                low_expressed = all(
                    [
                        value < 10
                        for value in parsed_profile["raw_data"].values()
                ...
                min_value = min(
                    [
                        ...
                        for i in parsed_profile["raw_data"].values()
                    ]
                )
                max_value = max(
                    [
                ...
                if max_value > 0:
                    for c in self.conditions:
                        if parsed_profile["raw_data"][c] is not None:
                            parsed_profile["data"][c] = (
                                parsed_profile["raw_data"][c] / max_value
                            )
                        else:
            ...
            output["heatmap_data"].append(
                {
                    ...
                    "values": {
                        ...
                        for k, v in d["profile"][key].items()
                    },
                }
            )

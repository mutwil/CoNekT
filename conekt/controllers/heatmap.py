...
def heatmap_cluster(cluster_id, option="zlog"):
    ...
    associations = SequenceCoexpressionClusterAssociation.query.filter_by(
        coexpression_cluster_id=cluster_id
    ).all()

    probes = [a.probe for a in associations]

    current_heatmap = ExpressionProfile.get_heatmap(
        cluster.method.network_method.species_id,
        probes,
        zlog=(option == "zlog"),
        raw=(option == "raw"),
    )
...
def heatmap_main():
    ...
    example = {"species_id": None, "probes": None, "options": "zlog"}
    ...
    for s_id in species_ids[:3]:
        profiles = (
            ExpressionProfile.query.filter(ExpressionProfile.sequence_id is not None)
            .filter(ExpressionProfile.species_id == s_id)
            .limit(3)
            .all()
        )
        for p in profiles:
...
def heatmap_custom_default():
    ...
    current_heatmap = ExpressionProfile.get_heatmap(
        species_id, probes, zlog=(option == "zlog"), raw=(option == "raw")
    )
...
def heatmap_custom_comparable():
    ...
    current_heatmap = CrossSpeciesExpressionProfile().get_heatmap(
        *sequence_ids, option=option
    )
...
def heatmap_comparative_tree(tree_id, option="raw"):
    ...
    heatmap_data = CrossSpeciesExpressionProfile().get_heatmap(
        *sequence_ids, option=option
    )

    return render_template(
        "expression_heatmap.html",
        order=heatmap_data["order"],
...
def heatmap_inchlib_json(cluster_id):
    ...
    current_heatmap = ExpressionProfile.get_heatmap(
        cluster.method.network_method.species_id, probes
    )
        ...
        "data": {
            "nodes": {
                a["name"]: {
                    ...
                    "distance": 0,
                    "features": [
                        round(a["values"][o], 3) if a["values"][o] != "-" else None
                        for o in current_heatmap["order"]
                    ],
                    "objects": [a["name"]],
                }

...
def configure_admin_panel(app):
    ...
    if LOGIN_ENABLED:
        ...
        from conekt.controllers.admin.views.expression_profiles import (
            AddExpressionProfilesView,
        )
        from conekt.controllers.admin.views.expression_networks import (
            AddCoexpressionNetworkView,
        )
        from conekt.controllers.admin.views.expression_networks import (
            ExpressionNetworkMethodAdminView,
        )
        from conekt.controllers.admin.views.expression_specificity import (
            AddSpecificityView,
        )
        from conekt.controllers.admin.views.expression_specificity import (
        ...
        from conekt.controllers.admin.views.expression_specificity import (
            ExpressionSpecificityMethodAdminView,
        )
        ...
        from conekt.controllers.admin.views.families import (
            AddFamiliesView,
            AddFamilyAnnotationView,
        )
        ...
        from conekt.controllers.admin.views.expression_clusters import (
            BuildNeighorhoodToClustersView,
        )
        from conekt.controllers.admin.views.expression_clusters import (
            BuildCoexpressionClustersView,
        )
        from conekt.controllers.admin.views.expression_clusters import (
            AddCoexpressionClustersView,
        )
        from conekt.controllers.admin.views.expression_clusters import (
            ClusterSimilaritiesView,
        )
        from conekt.controllers.admin.views.expression_clusters import (
        ...
        admin.add_view(
            AddSequenceDescriptionsView(
                ...
                url="add/sequence_descriptions/",
                category="Add",
            )
        )

        admin.add_menu_item(
            MenuLink("------------", class_name="divider", url="#"),
            target_category="Add",
        )
        admin.add_menu_item(
            MenuLink("Functional Annotation", class_name="disabled", url="#"),
            target_category="Add",
        )

        admin.add_view(
            AddFunctionalDataView(
                name="GO and InterPro definitions",
                endpoint="admin.add.functional_data",
                url="add/functional_data/",
                category="Add",
            )
        )
        ...
        admin.add_menu_item(
            MenuLink("Expression", class_name="disabled", url="#"),
            target_category="Add",
        )
        ...
        admin.add_view(
            AddCoexpressionClustersView(
                name="Coexpression clusters",
                endpoint="admin.add.coexpression_clusters",
                url="add/coexpression_clusters/",
                category="Add",
            )
        )
        ...
        admin.add_view(
            AddSpecificityView(
                name="Expression Specificity",
                endpoint="admin.add.expression_specificity",
                url="add/expression_specificity/",
                category="Add",
            )
        )
        ...
        admin.add_menu_item(
            MenuLink("Comparative Genomics", class_name="disabled", url="#"),
            target_category="Add",
        )
        ...
        admin.add_menu_item(
            MenuLink("Misc.", class_name="disabled", url="#"), target_category="Add"
        )
        ...
        admin.add_menu_item(
            MenuLink(
                "Update Counts",
                url="/admin_controls/update/counts",
        ...
        admin.add_menu_item(
            MenuLink(
                "Assign Clades",
                url="/admin_controls/update/clades",
                class_name="confirmation",
            ),
            target_category="Build",
        )
        ...
        admin.add_view(
            AddFamilyAnnotationView(
                name="Family-wise annotation",
                endpoint="admin.add.family_annotation",
                url="build/family_annotation/",
                category="Build",
            )
        )
        ...
        admin.add_view(
            ReconcileTreesView(
                name="Reconcile Trees",
                endpoint="admin.reconcile_trees",
                url="build/reconciled_trees",
                category="Build",
            )
        )
        ...
        admin.add_view(
            ECCView(
                name="Expression Context Conservations (ECC)",
                endpoint="admin.ecc",
                url="build/ecc/",
                category="Build",
            )
        ...
        admin.add_menu_item(
            MenuLink("Co-expression Clusters", class_name="disabled", url="#"),
            target_category="Build",
        )

        admin.add_view(
            ClusterSimilaritiesView(
                name="Cluster Similarities",
                endpoint="admin.clustersimilarities",
        ...
        admin.add_view(
            GOEnrichmentView(
                name="Cluster GO Enrichment",
                endpoint="admin.goenrichment",
        ...
        admin.add_view(
            BuildCoexpressionClustersView(
                ...
                url="build/hcca_clusters/",
                category="Build",
            )
        )
        admin.add_view(
            BuildNeighorhoodToClustersView(
                name="Neighborhood to clusters",
                endpoint="admin.build.neighborhood_to_clusters",
                url="build/neighborhood_to_clusters/",
                category="Build",
            )
        )
        admin.add_menu_item(
        ...
        admin.add_view(
            PredictGOView(
                name="Predict GO from neighborhood",
                endpoint="admin.predict.go",
                url="predict/go",
                category="Build",
            )
        )
        ...
        # Control panel
        admin.add_view(
            ControlsView(name="Controls", endpoint="admin.controls", url="controls/")
        )

        # CRUD for various database tables
        admin.add_view(
            NewsAdminView(
                News, db.session, endpoint="admin.news", url="news", category="Browse"
            )
        )
        admin.add_view(
            SpeciesAdminView(Species, db.session, url="species", category="Browse")
        )
        admin.add_view(
            CladesAdminView(
                Clade, db.session, url="clades", category="Browse", name="Clades"
            )
        )
        admin.add_view(
            ConditionTissueAdminView(
                ConditionTissue,
                db.session,
                url="condition_tissue/",
                category="Browse",
                name="Condition to Tissue",
            )
        )

        admin.add_menu_item(
            MenuLink("------------", class_name="divider", url="#"),
            target_category="Browse",
        )
        admin.add_menu_item(
            MenuLink("Methods", class_name="disabled", url="#"),
            target_category="Browse",
        )

        admin.add_view(
            GeneFamilyMethodAdminView(
                GeneFamilyMethod,
                db.session,
                url="families",
                category="Browse",
...
def configure_error_handlers(app):
    ...
    def handle_csrf_error(e):
        flash(
            "Could not handle request, CSRF token has expired. Please try again...",
            "warning",
        )
...
def configure_hooks(app):
    ...
    BLAST_ENABLED = app.config["BLAST_ENABLED"]
    TWITTER_HANDLE = (
        app.config["TWITTER_HANDLE"] if "TWITTER_HANDLE" in app.config.keys() else None
    )
    TUTORIAL_URL = (
        app.config["TUTORIAL_URL"] if "TUTORIAL_URL" in app.config.keys() else None
    )
    IMPRINT = app.config["IMPRINT_URL"] if "IMPRINT_URL" in app.config.keys() else None
    PRIVACY = (
        app.config["PRIVACY_POLICY_URL"]
        if "PRIVACY_POLICY_URL" in app.config.keys()
        else None
    )
    ...
    def before_request():
        ...
        if "GLOB_MSG" in app.config and app.config["GLOB_MSG"] is not None:
            ...
            g.msg_title = (
                app.config["GLOB_MSG_TITLE"]
                if "GLOB_MSG_TITLE" in app.config
                else "info"
            )
        else:
            g.msg = None
